...
class FormatterFrTest {
    ...
    public static class MyPackedRangeTest {
        @Test
        void testPackValidValues() {
            assertEquals(305419904, PackedRange.pack(1193046, 1193174)); // Longueur = 128
            assertEquals(0x00FFFF05, PackedRange.pack(0x00FFFF, 0x010004)); // Longueur = 5
        }

        @Test
        void testPackInvalidValues() {
            assertThrows(IllegalArgumentException.class, () -> PackedRange.pack(0x1000000, 0x1000001)); // Dépasse 24 bits
            assertThrows(IllegalArgumentException.class, () -> PackedRange.pack(0x00FFFF, 0x011000)); // Longueur dépasse 8 bits
        }

        @Test
        void testStartInclusive() {
            assertEquals(0x123456, PackedRange.startInclusive(0x12345678));
            assertEquals(0x00FFFF, PackedRange.startInclusive(0x00FFFF05));
            assertEquals(0xFFFFFF, PackedRange.startInclusive(0xFFFFFF7F));
        }

        @Test
        void testLength() {
            assertEquals(0x78, PackedRange.length(0x12345678));
            assertEquals(0x05, PackedRange.length(0x00FFFF05));
            assertEquals(0x7F, PackedRange.length(0xFFFFFF7F));
        }

        @Test
        void testEndExclusive() {
            //assertEquals(0x1234D6, PackedRange.endExclusive(0x12345678)); // 0x123456 + 0x78
            assertEquals(0x010004, PackedRange.endExclusive(0x00FFFF05)); // 0x00FFFF + 5
            assertEquals(16777342, PackedRange.endExclusive(0xFFFFFF7F)); // 0xFFFFFF + 0x7F
        }
    }
}