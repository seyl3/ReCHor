...
public record DetailUI(Node rootNode) {
    ...
    public static DetailUI create(ObservableValue<Journey> journey) {
        ...
        for (Journey.Leg leg : journey.getValue().legs()) {
            switch (leg) {
                ...
                case Journey.Leg.Transport transportLeg -> {
                    ...
                    // Ajout de l'heure de départ, cercle de départ, nom de la gare et sa plateforme
                    // de départ
                    ...
                    // Ajout de l'heure d'arrivée, cercle d'arrivée, nom de la gare et sa plateforme
                    // d'arrivée
        ...
        boolean hasJourney = journey.getValue() != null;
        noJourney.setVisible(!hasJourney);
        withJourney.setVisible(hasJourney);
        ...
        calendarButton.setOnAction(a -> {
            ...
            File selectedFile = fileChooser.showSaveDialog(calendarButton.getScene().getWindow());

            if (selectedFile != null) {
                try {
                    String icalText = toIcalendar(journey.getValue());

                    Files.writeString(selectedFile.toPath(), icalText, StandardOpenOption.CREATE);
                } catch (IOException e) {
